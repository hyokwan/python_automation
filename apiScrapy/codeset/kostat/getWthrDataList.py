...
if org_data.empty:
    startDt = endDt - relativedelta(years=10)
else:
    startDt = endDt - relativedelta(years=1)

if not org_data.empty:
    org_data = org_data.loc[org_data.시간 < startDt.strftime("%Y-%m-%d")]
...
BASEPARAM_VAL = [numOfRows, dataCd, dateCd, startDt.strftime("%Y%m%d"), endDt.strftime("%Y%m%d")]
...
if completed:
    ...
    index = \
    clc_typcial[(clc_typcial.ctgry_nm == "통계청") & (clc_typcial.strg_filename == f'{DATANAME}.csv')].index.values[0]
...
if completed:
    ...
    #     mergedDf.sort_values(by=["입찰공고일시"], inplace=True)
